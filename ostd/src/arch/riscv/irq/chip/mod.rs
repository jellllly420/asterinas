    ...
    pub(in crate::arch) fn claim_interrupt(&self, hart: u32) -> Option<HwIrqLine> {
        self.plics
            ...
            .find_map(|(index, plic)| {
                ...
                let irq_num = plic.interrupt_number_mappings().get(interrupt as usize)?;
                irq_num.map(|irq_num| HwIrqLine {
                    irq_num: irq_num as u8,
                    source: InterruptSource::External(InterruptSourceOnChip { index, interrupt }),
                })
            })
    }
